...
for Did=1:size(Darr,2)
    ...
    fname = ['simplify_M7_CEEC2017_',num2str(D),'D.txt'];
    f_out = fopen(fname,'wt');
    fname_std = ['simplify_M7_CEEC2017_std',num2str(D),'D.txt'];
    f_out_std = fopen(fname_std,'wt');
    fMedian = ['M7SHADE_',num2str(D),'D_locMedian.txt'];
    ...
    for ifun=1: fun_nums
        ...
        for run=1:runnum
            ...
            name = ['CAEA_fid_',num2str(ifun),'_',num2str(D),'D_',num2str(run),'.dat'];
            output = fopen(name,'a');
            %---------------Initialization-----------------
            %parameter setting
            %population size
            ...
            while(FES < maxfe)
                ...
                if (mod(subgen, num_gen) == 0 && subgen>1 ) || (FES == maxfe)
                    ...
                    if ~isempty(p_app)==1
                        ...
                        FES=FES+1;
                        %                          if((mod(FES-ps,floor(maxfe/20))>=floor(maxfe/20)-ps)&& mod(FES,floor(maxfe/20))<ps)
                ...
                %         if((mod(fitcount-ps,floor(maxfe/20))>=floor(maxfe/20)-ps)&& mod(fitcount,100*5*D)<ps)
                %         if mod(fitcount-ps,floor(maxfe/20))==0


            end
            fprintf(f_out,'x[%s]=%s\n',num2str(gbpos),num2str(bestever));
            bestval(ifun,run) =  bestever;
